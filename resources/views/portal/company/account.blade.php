...
<div>
    ...
    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Dados de quem entra neste painel pela {{ $user->company->name }}.</p>
</div>

<div class="grid max-w-4xl gap-6 lg:grid-cols-2">
    <form method="post" action="{{ route('empresa.conta.update') }}" class="space-y-4">
        @csrf
        @method('put')
    ...
    <form method="post" action="{{ route('empresa.conta.senha') }}" class="space-y-4">
        @csrf
        @method('put')
