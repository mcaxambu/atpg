...
@php
    ...
    $action = route('membro.perfil.update');
    $selected = old('specialties', $member->specialties->pluck('id')->all());
    $lines = fn ($collection) => $collection->pluck('title')->implode("\n");
@endphp

<div>
    <h1 class="text-2xl font-semibold text-gray-800 dark:text-white/90">Meu cadastro</h1>
    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
        É este o perfil que aparece no diretório da associação. Toda alteração volta para análise antes de ir ao ar.
    </p>
</div>

@if ($member->isPending())
    <div class="rounded-lg border border-warning-500/30 bg-warning-50 px-4 py-3 text-sm text-warning-700 dark:bg-warning-500/10 dark:text-warning-400">
        Seu cadastro está em análise pela associação.
    </div>
@endif
