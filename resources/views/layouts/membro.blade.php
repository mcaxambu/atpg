@php
    ...
    $menu = [
        ['label' => 'Meu cadastro', 'icon' => 'users', 'route' => route('membro.perfil.edit'), 'active' => request()->routeIs('membro.perfil.*')],
        // Só aparece para quem foi marcado como colunista no cadastro.
        ...(auth()->user()?->columnist()
            ? [['label' => 'Minhas colunas', 'icon' => 'news', 'route' => route('membro.colunas.index'), 'active' => request()->routeIs('membro.colunas.*')]]
            : []),
        ['label' => 'Meu acesso', 'icon' => 'shield', 'route' => route('membro.conta.edit'), 'active' => request()->routeIs('membro.conta.*')],
    ];
@endphp
<!doctype html>
...
<head>
    ...
    <title>@yield('title', 'Painel') | {{ $member?->name ?? 'Membro' }}</title>
...
<body x-data class="bg-gray-50 font-outfit text-gray-800 dark:bg-gray-900 dark:text-gray-200">
    <header class="sticky top-0 z-9999 border-b border-gray-200 bg-white/95 backdrop-blur dark:border-gray-800 dark:bg-gray-900/95">
        <div class="mx-auto flex max-w-(--breakpoint-xl) items-center justify-between gap-4 px-4 py-3">
            {{--
                Duas identidades: a do portal (logo da ATPG) e a do membro que
                está logado. Separadas por um divisor para não parecerem a
                mesma marca.
            --}}
            <div class="flex min-w-0 items-center gap-3">
                ...
                <a href="{{ route('membro.perfil.edit') }}" class="flex min-w-0 items-center gap-2.5">
                    <x-admin.avatar :photo="$member?->photo_path" :initials="$member?->avatar_initials ?? '?'"
                                    size="h-9 w-9" />
                    <span class="min-w-0 leading-tight">
                        <strong class="block truncate text-sm font-semibold text-gray-900 dark:text-white">{{ $member?->name }}</strong>
                        <small class="text-xs text-gray-500 dark:text-gray-400">Painel do membro</small>
                    </span>
                </a>
            </div>
            ...
            <div class="flex items-center gap-2">
                ...
                @if ($member?->isPubliclyVisible())
                    <a href="{{ route('members.show', $member) }}" target="_blank" rel="noopener"
